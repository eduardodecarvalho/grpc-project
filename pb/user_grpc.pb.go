...
type UserServiceClient interface {
	...
	AddUserBidirectional(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserBidirectionalClient, error)
}
...
func (c *userServiceClient) AddUserBidirectional(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserBidirectionalClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUserBidirectional", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUserBidirectionalClient{stream}
	return x, nil
}

type UserService_AddUserBidirectionalClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUserBidirectionalClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUserBidirectionalClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUserBidirectionalClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type UserServiceServer interface {
	...
	AddUserBidirectional(UserService_AddUserBidirectionalServer) error
...
func (UnimplementedUserServiceServer) AddUserBidirectional(UserService_AddUserBidirectionalServer) error {
...
func _UserService_AddUserBidirectional_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUserBidirectional(&userServiceAddUserBidirectionalServer{stream})
}

type UserService_AddUserBidirectionalServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUserBidirectionalServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUserBidirectionalServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUserBidirectionalServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
